...
class Task:
    ...
    def get_reward(self):
        ...
        current_x, current_y, current_z = self.sim.pose[:3]
        target_x, target_y, target_z = self.target_pos
        x_part = max(current_x, target_x) - min(current_x, target_x)
        y_part = max(current_y, target_y) - min(current_y, target_y)
        z_part = max(current_z, target_z) - min(current_z, target_z)
        distance_from_target = np.sqrt(x_part**2 + y_part**2 + z_part**2)
        self.distance_from_target = distance_from_target
        return 1 / (distance_from_target+0.00001) * 1000
